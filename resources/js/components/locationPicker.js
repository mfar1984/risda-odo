...
export default function locationPicker(mapId, provider = 'maptiler', apiKey = null, styleUrl = null, options = {}) {
    ...
    return {
        ...
        createLayers() {
            if (this.effectiveProvider === 'maptiler') {
                ...
                const streetLayer = buildMaptilerTileUrl(streetStyle, this.apiKey)
                    ? L.tileLayer(buildMaptilerTileUrl(streetStyle, this.apiKey), {
                          attribution,
                          maxZoom: 19,
                          crossOrigin: true,
                      })
                ...
                const satelliteLayer = buildMaptilerTileUrl(satelliteStyle, this.apiKey)
                    ? L.tileLayer(buildMaptilerTileUrl(satelliteStyle, this.apiKey), {
                          attribution,
                          maxZoom: 19,
                          crossOrigin: true,
                      })
                ...
                const hybridLayer = buildMaptilerTileUrl(hybridStyle, this.apiKey)
                    ? L.tileLayer(buildMaptilerTileUrl(hybridStyle, this.apiKey), {
                          attribution,
                          maxZoom: 19,
                          crossOrigin: true,
                      })
            ...
            return {
                ...
                satellite: L.tileLayer(
                    ...
                    {
                        attribution:
                            '© Esri — Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, dan komuniti GIS',
                        maxZoom: 19,
                    }
                ),
            };
        ...
        updateCoordinates(lat, lng, options = {}) {
            ...
            if (options.pan && this.map) {
                const zoom = options.zoom ?? this.map.getZoom();
