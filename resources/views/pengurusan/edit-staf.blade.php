<x-dashboard-layout 
            ...
            <section>
                <header>
                    ...
                    <p class="mt-1 text-sm text-gray-600">
                        {{ __('Kemaskini maklumat RISDA Staf') }}
                    </p>
                </header>

                <form method="POST" action="{{ route('pengurusan.update-staf', $risdaStaf) }}" class="mt-6 space-y-6" x-data="stafHandler()">
                    @csrf
                    @method('PUT')
                    ...
                    <div style="display: flex; gap: 20px;">
                        <div style="flex: 1;">
                            ...
                            >
                                ...
                                <option value="{{ $bahagian->id }}" {{ old('bahagian_id', $risdaStaf->bahagian_id) == $bahagian->id ? 'selected' : '' }}>
                    ...
                    <div style="display: flex; gap: 20px;">
                        <div style="flex: 1;">
                            ...
                            <select id="status" name="status" class="form-select mt-1" required>
                                <option value="aktif" {{ old('status', $risdaStaf->status) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                                <option value="tidak_aktif" {{ old('status', $risdaStaf->status) == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                                <option value="gantung" {{ old('status', $risdaStaf->status) == 'gantung' ? 'selected' : '' }}>Gantung</option>
                            </select>
    ...
    <script>
        function stafHandler() {
            return {
                bahagianId: '{{ old('bahagian_id', $risdaStaf->bahagian_id) }}',
                stesenId: '{{ old('stesen_id', $risdaStaf->stesen_id) }}',
                availableStesen: [],
                ...
                loadStesen() {
                    ...
                    if (this.bahagianId) {
                        ...
                        this.availableStesen.forEach(stesen => {
                            const selected = stesen.id == this.stesenId ? 'selected' : '';
                            this.stesenOptions += `<option value="${stesen.id}" ${selected}>${stesen.nama_stesen}</option>`;
                        });
                    }
                },
